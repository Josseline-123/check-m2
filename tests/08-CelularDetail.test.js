...
describe('<CelularDetail />', () => {
   ...
   it('Debe utilizar React.useEffect para que despache la acción "getCelularesDetails", pasándole como argumento el ID del celular', async () => {
      // Nuevamente testeamos todo el proceso. Tenes que usar un useEffect, y despachar la acción "getCelularesDetails".
      const useDispatch = jest.spyOn(ReactRedux, 'useDispatch');
      const getCelularesDetails = jest.spyOn(actions, 'getCelularesDetails');
      celularDetail(1);
      expect(useEffect).toHaveBeenCalled();
      expect(useDispatch).toHaveBeenCalled();
      expect(getCelularesDetails).toHaveBeenCalled();
   ...
   describe('Debe utilizar el "id" de "params" para despachar la action "getCelularesDetails" y renderizar los detalles del celular', () => {
      ...
      it("Debe renderizar un tag 'h1' que muestre el modelo de cada 'celular'", () => {
         useSelectorFn(1);
         expect(celularDetail(1).text().includes(celulares.modelo)).toEqual(
            true
         );
         expect(celularDetail(1).find('h1').at(0).text()).toBe(
            celulares.modelo
         );
      ...
      it("Debe renderizar una etiqueta 'img' donde su prop 'src' sea la imagen del celular y la prop 'alt' el modelo del celular.", () => {
         useSelectorFn(1);
         expect(celularDetail(1).find('img').prop('src')).toBe(
            celulares.imagen
         );
         expect(celularDetail(1).find('img').prop('alt')).toBe(
            celulares.modelo
         );
         expect(useSelectorStub).toHaveBeenCalled();
         expect(useEffect).toHaveBeenCalled();
      ...
      it("Debe renderizar una etiqueta 'h3' que contenga el texto 'Precio: ' y el precio del celular.", () => {
         useSelectorFn(1);
         expect(celularDetail(1).text().includes(celulares.precio)).toEqual(
            true
         );
      ...
      it("Debe renderizar una etiqueta 'h5' que contenga el texto 'Marca: ' y la marca del celular.", () => {
         useSelectorFn(1);
         expect(celularDetail(1).text().includes(celulares.marca)).toEqual(
            true
         );
      ...
      it("Debe renderizar una etiqueta 'h5' que contenga el texto 'Lanzamiento: ' y el Lanzamiento del celular.", () => {
         useSelectorFn(1);
         expect(
            celularDetail(1).text().includes(celulares.lanzamiento)
         ).toEqual(true);
      ...
      it("Debe renderizar una etiqueta 'h5' que contenga el texto 'Descripción: ' y la Descripción del celular.", () => {
         useSelectorFn(1);
         expect(
            celularDetail(1).text().includes(celulares.descripción)
         ).toEqual(true);
